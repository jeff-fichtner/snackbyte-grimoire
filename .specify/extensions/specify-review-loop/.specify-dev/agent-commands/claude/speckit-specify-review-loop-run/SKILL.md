---
name: speckit-specify-review-loop-run
description: Recursively review an artifact (spec or code) for issues and fix the
  unambiguous ones until clean or blocked on the user
compatibility: Requires spec-kit project structure with .specify/ directory
metadata:
  author: github-spec-kit
  source: specify-review-loop:commands/speckit.specify-review-loop.run.md
---
...
1. **Review** the target artifact for issues:
   ...
   - **code**: correctness against the spec/plan/tasks, obvious bugs, dead or contradictory code,
     ...
     (e.g. deterministic logic that belongs in a tested `.specify/extensions/specify-review-loop/scripts/bash/*.sh`), and anything the
